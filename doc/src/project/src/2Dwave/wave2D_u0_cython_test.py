...
def solver(I, V, f, c, Lx, Ly, Nx, Ny, dt, T,
    ...
    if version == 'cython':
        try:
            ...
            print e
            sys.exit(1)
    elif version == 'pcython':
        try:
            #import pyximport; pyximport.install()
            import wave2D_u0_par_loop_cy as compiled_loops
            advance = compiled_loops.advance
        except ImportError as e:
            print 'No module wave2D_u0_par_loop_cy. Run make_wave2D.sh!'
            print e
            sys.exit(1)
    elif version == 'f77':
        try:
    ...
    if version == 'scalar':
        ...
        V_a = V(xv, yv)
        u = advance_jit(u, u_1, u_2, f_a, V_a, Cx2, Cy2, Nx, Ny, n, dt, True)
...
def advance_jit(u, u_1, u_2, f, V, Cx2, Cy2, Nx, Ny, n, dt, step1):
    Ix = range(0, u.shape[0])
    Iy = range(0, u.shape[1])
    dt2 = dt**2
...
def advance_numexpr(u, u_1, u_2, f_a, Cx2, Cy2, dt2, V=None, step1=False):
    if step1:
        dt = sqrt(dt2)
        Cx2 = 0.5*Cx2;  Cy2 = 0.5*Cy2; dt2 = 0.5*dt2
        D1 = 1;  D2 = 0
